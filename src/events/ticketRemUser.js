    ...
    try {
        ...
        if(interaction.isButton()) {
            if (interaction.customId === "ticket-remuser") {
               ...
               const remusername = new TextInputBuilder()
                    .setCustomId("ticket-remuser-id")
                    .setLabel("User ID")
                    .setPlaceholder("Gib die User ID des Users an.")
                    .setStyle(TextInputStyle.Short)
                    .setRequired(true);

                const UserNameInput = new ActionRowBuilder().addComponents(remusername);
        ...
        if(interaction.isModalSubmit()) {
            if (interaction.customId.startsWith("ticket-remuser-modal")) {
                const channel = interaction.guild.channels.cache.get(interaction.channel.id);
                const remuserID = interaction.fields.getTextInputValue('ticket-remuser-id');
                ...
                DBconnection.query("SELECT * from `" + channel.id + "`", async function (err, results, fields) {
                    if (err) {
                        ...
                        .setAuthor({ name: embed_author_text, iconURL: embed_author_icon })
                        .setDescription("### Der Nutzer <@" + remuserID + "> wurde entfernt!")
                        .setTimestamp()
                        .setFooter({ text: embed_footer_text, iconURL: embed_footer_icon })
                        .setColor(embed_color)
