        ...
        try {
            if (interaction.isButton()) {
                if (interaction.customId === "ticketClaim") {
                    ...
                    UpdateTicket(interaction, "ClaimerID", interaction.user.id, interaction.channel.id);

                    let ticketUser = interaction.guild.members.cache.find(user => user.id === ticketData[0].UserID)

                    let newChannelName = `【${SupportConfiguration.ticket_types[ticketData[0].Category].Emoji}】${ticketUser.user.username}-${interaction.user.username}`;
                    interaction.channel.setName(newChannelName);

                    let embedMessage = await SupportEmbedBuilder(`## Das Ticket wurde von <@${interaction.user.id}> übernommen!`);
